...
ENTITY_REGISTRY = [
    ...
    ("charging", "binary", "binary_sensor_charging", {"name": "Charging", "icon": "mdi:power-plug-battery", "device_class": "battery_charging"}),
    ("low_power_mode", "binary", "binary_sensor_low_power_mode", {"name": "Low power mode"}),
    ("high_speed_mode", "binary", "binary_sensor_high_speed_mode", {"name": "High speed mode"}),
    ("firmware_version", "text", "text_sensor_firmware_version", {"name": "Firmware Version", "icon": "mdi:cellphone-arrow-down"}),
    ("auto_off", "sensor", "sensor_auto_off", _sensor_defaults("Auto Off", "mdi:progress-clock", "duration", "measurement", "sec", 0)),
    ("battery_percentage", "sensor", "sensor_battery_percentage", _sensor_defaults("Battery", "mdi:battery", "battery", "measurement", "%", 1)),
    ("bms_left_current", "sensor", "sensor_bms_left_current", _sensor_defaults("BMS Left Current", "mdi:current-ac", "current", "measurement", "A", 3)),
    ("bms_right_current", "sensor", "sensor_bms_right_current", _sensor_defaults("BMS Right Current", "mdi:current-ac", "current", "measurement", "A", 3)),
    ("power", "sensor", "sensor_power", _sensor_defaults("Power", "mdi:lightning-bolt", "power", "measurement", "W", 3)),
    ("temperature_motor", "sensor", "sensor_temperature_motor", _sensor_defaults("Temperature motor", "mdi:thermometer", "temperature", "measurement", "°C", 2)),
    ("temperature_controller", "sensor", "sensor_temperature_controller", _sensor_defaults("Temperature controller", "mdi:thermometer", "temperature", "measurement", "°C", 2)),
    ("tho_ra", "sensor", "sensor_tho_ra", _sensor_defaults("Tho_ra", state_class="measurement", unit_of_measurement="%", accuracy_decimals=0)),
    ("mileage_current", "sensor", "sensor_mileage_current", _sensor_defaults("Mileage current", "mdi:map-marker-path", "distance", "measurement", "km", 2)),
    ("mileage_total", "sensor", "sensor_mileage_total", _sensor_defaults("Mileage total", "mdi:map-marker-distance", "distance", "measurement", "km", 2)),
    ("voltage", "sensor", "sensor_voltage", _sensor_defaults("Voltage", "mdi:flash", "voltage", "measurement", "V", 2)),
    ("bms_temperature_min", "sensor", "sensor_bms_temperature_min", _sensor_defaults("BMS Temperature Min", "mdi:thermometer-low", "temperature", "measurement", "°C", 2)),
    ("bms_temperature_max", "sensor", "sensor_bms_temperature_max", _sensor_defaults("BMS Temperature Max", "mdi:thermometer-high", "temperature", "measurement", "°C", 2)),
    ("bms_cell_voltage_min", "sensor", "sensor_bms_cell_voltage_min", _sensor_defaults("BMS Cell Voltage Min", "mdi:battery-minus", "voltage", "measurement", "V", 3)),
    ("bms_cell_voltage_max", "sensor", "sensor_bms_cell_voltage_max", _sensor_defaults("BMS Cell Voltage Max", "mdi:battery-plus", "voltage", "measurement", "V", 3)),
    ("bms_cell_voltage_delta", "sensor", "sensor_bms_cell_voltage_delta", _sensor_defaults("BMS Cell Voltage Delta", "mdi:battery-arrow-up-down", "voltage", "measurement", "V", 3)),
    ("headlight", "text", "text_sensor_headlight", {"name": "Headlight", "icon": "mdi:lightbulb"}),
]
for i in range(1, 7):
    ENTITY_REGISTRY.append((
        f"bms_left_temp_{i}", "sensor", f"sensor_bms_left_temp_{i}",
        _sensor_defaults(f"BMS Left Temperature {i}", "mdi:thermometer", "temperature", "measurement", "°C", 2),
    ))
    ENTITY_REGISTRY.append((
        f"bms_right_temp_{i}", "sensor", f"sensor_bms_right_temp_{i}",
...
WEB_SERVER_DEFAULTS = {
    "auto_off": (CONF_SORTING_GROUP_ID, 41),
    "temperature_motor": (CONF_SORTING_GROUP_ID, 22),
    "temperature_controller": (CONF_SORTING_GROUP_ID, 23),
    "bms_left_current": (CONF_SORTING_GROUP_ID, 14),
    "bms_right_current": (CONF_SORTING_GROUP_ID, 15),
    "power": (CONF_SORTING_GROUP_ID, 13),
    "tho_ra": (CONF_SORTING_GROUP_ID, 42),
    ...
    "firmware_version": (CONF_SORTING_GROUP_ID, 53),
    "low_power_mode": (CONF_SORTING_GROUP_ID, 51),
    "high_speed_mode": (CONF_SORTING_GROUP_ID, 52),
    "headlight": (CONF_SORTING_GROUP_ID, 62),
    "bms_temperature_min": (CONF_SORTING_GROUP_ID, 30),
    "bms_temperature_max": (CONF_SORTING_GROUP_ID, 31),
    "bms_cell_voltage_min": (CONF_SORTING_GROUP_ID, 34),
    "bms_cell_voltage_max": (CONF_SORTING_GROUP_ID, 35),
    "bms_cell_voltage_delta": (CONF_SORTING_GROUP_ID, 36),
}
for i in range(1, 7):
    WEB_SERVER_DEFAULTS[f"bms_left_temp_{i}"] = (CONF_SORTING_GROUP_ID, 31 + i)
    WEB_SERVER_DEFAULTS[f"bms_right_temp_{i}"] = (CONF_SORTING_GROUP_ID, 37 + i)
